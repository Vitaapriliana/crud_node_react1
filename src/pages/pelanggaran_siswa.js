...
class PelanggaranSiswa extends React.Component {
    ...
    getMurid = () => {
        ...
        // mengakses api untuk mengambil data murid
        axios.get(url, this.headerConfig())
        .then(response => {
          // mengisikan data dari respon API ke array murid
        ...
        return(
            ...
            <Modal show={this.state.isModal3Open} onHide={this.handleClose}>
                ...
                <Form onSubmit={this.handleSave}>
                    <Modal.Body>
                    Nama Siswa  
                        <select name="id_murid" className="form-control" value={this.state.id_murid}  
                        onChange={this.bind} required>  
                        <option value="">-- Pilih Siswa --</option>  
                            { this.state.murid.map(item => {  
                                return (  
                                <option key={item.id_murid} value={item.id_murid}>  
                                    {item.nama_murid}  
                                </option>  
                                );  
                            }) }  
